...
![Checks](https://img.shields.io/badge/checks-61-blue)
...
A curation pass extending CVE coverage and correcting the io_uring set. **CVE database 43 → 51 entries**; check count unchanged at **60**. Conf-only — no script-logic change, so this ships under script **v4.7.9**. Relevant container-escape and privilege-escalation CVEs are now **backdated to 2024**, with landmark escapes earlier still covered (DirtyCOW 2016, runc CVE-2019-5736, DirtyPipe / cgroup release_agent 2022, OverlayFS 2023). The **Recent CVEs table is brought to full parity** with the database — all 51 entries are now listed, including SCTPhantom and Zapscape, which had been documented in prose only since v4.7.8.

- **runc trio promoted to first-class CVE entries** — CVE-2025-31133 (maskedPaths `/dev/null` symlink), CVE-2025-52565 (`/dev/console` bind-mount race), CVE-2025-52881 (`/proc` write-redirect / LSM-bypass). Previously covered only behaviourally by checks 26/52; now `component=runc` entries (fixed runc 1.2.8 / 1.3.3 / 1.4.0-rc.3) that defer to those checks for the live verdict. CVE-2025-52881 remains the LSM-bypass tie-in for checks 57/58 via `RUNC_LSM_BYPASS`.
- **ksmbd LPE added** — CVE-2025-37947, an out-of-bounds write in `ksmbd_vfs_stream_write()` with a public working exploit (Doyensec), module-gated on `ksmbd` / `streams_xattr`. Pairs with the existing ksmbd entry CVE-2022-47939.
- **2024 backdate** — CVE-2024-0582, the io_uring `IORING_REGISTER_PBUF_RING` page-reuse use-after-free (Jann Horn / Project Zero) with a public weaponised exploit (Exodus, "Mind the Patch Gap"); introduced 6.4, fixed 6.6.5. A genuine HIGH io_uring LPE.
- **io_uring correction (important).** An earlier draft added three io_uring CVEs that a verification step then re-examined. **CVE-2025-21333 was removed** — it is a *Microsoft Windows Hyper-V* CVE, not Linux (a source misattribution); keeping it would have flagged every Linux host with a Windows bug. **CVE-2025-40364** (vendor-rated DoS — Amazon 3.3, availability-only) and **CVE-2025-21863** (Spectre-v1 opcode-speculation hardening, side-channel info-leak) were **re-added at honest LOW severity** under a conservative flag-if-present posture: retained so vulnerable code is surfaced, but scored to match what the bugs actually are rather than NVD's auto-7.8.
- **xfrm CVE** — CVE-2026-72137 (xfrm `nat_keepalive` double-free; local LPE via CAP_NET_ADMIN or an unprivileged user+net namespace) was folded in during this window.

> Every entry in this pass was verified individually against vendor / NVD data. Where NVD auto-scores diverge from vendor assessments — notably the two io_uring LOW entries and the runc trio — the entry records the dispute and uses the practical-impact value, so the tool's HIGH/CRITICAL findings continue to mean what they say. Table CVSS values marked `*` denote a disputed/vendor-corrected score explained in the notes below the table.

### v4.7.9 — KVM/x86 `/dev/kvm` reachability check (check 60) + Zapscape/Januscape correction

Adds check 60 (`check_kvm_x86_shadow_mmu_exposure`), the x86 parallel to check 51's arm64 ITScape probe: a read-only behavioural reachability signal for the x86 KVM shadow-MMU escapes (Zapscape CVE-2026-64561, Januscape CVE-2026-53359), which were previously version-only entries with no reachability probe. Check count **59 → 60**; CVE database unchanged at **43 entries**. No new dependencies; read-only throughout (`uname`, `stat`/`-w` on `/dev/kvm`, `/proc/modules`, `/sys/module/kvm_*/parameters/nested` — no `/dev/kvm` open, no KVM ioctls, no VM created).

- **New check 60 — `check_kvm_x86_shadow_mmu_exposure`.** x86-arch-gated (arm64 → not-applicable, deferring to check 51). Grades the reachability of the KVM shadow-MMU surface: `/dev/kvm` **writable from this context** → HIGH (the container-escape / LPE precondition; explicitly notes when the context is a container, where `/dev/kvm` should almost never be present); `/dev/kvm` **world-writable 0666** but not writable by the current user → HIGH (the RHEL / AlmaLinux / Rocky / CloudLinux-family LPE precondition for any local user); `/dev/kvm` present with **nested virt enabled** → MEDIUM (the classic guest→host path); locked-down + nested-off → OK. Cross-references both x86 KVM CVEs so those version-only entries now have a behavioural precondition, closing the arm64/x86 asymmetry (check 51 already did this for arm64).
- **Corrected the Zapscape (CVE-2026-64561) and Januscape (CVE-2026-53359) CVE entries.** Both previously framed these as nested-virt-only ("not reachable from a namespace container"). That understated the reach: on RHEL / AlmaLinux / Rocky / CloudLinux families `/dev/kvm` is world-writable (0666), so any local user — or a process in a container that has `/dev/kvm` exposed — can create a throwaway guest and drive the shadow-MMU use-after-free against the host kernel *without already being in a nested guest*. Both are therefore a direct unprivileged-to-root LPE on those distros and a container-to-host escape wherever `/dev/kvm` is reachable. The entries' `impact`/`exploit`/`rec` now lead with "don't expose `/dev/kvm` to containers" and "restrict `/dev/kvm` to a trusted group rather than 0666"; a dated `Corrected 2026-08-24:` note records the earlier misframing.

### v4.7.8 — SCTPhantom (CVE-2026-64564) + Zapscape (CVE-2026-64561) CVE-database entries

Two fresh container-relevant local-root LPEs disclosed on 2026-08-06, added to the config-driven CVE engine. No script-logic change — `cve_checks.conf` only. CVE database **41 → 43 entries**; check count unchanged at **59**.

- **CVE-2026-64564 (SCTPhantom).** Use-after-free in the SCTP stack's ASCONF (dynamic address reconfiguration) path: a crafted ordered DEL-IP sequence frees the cached transport then reuses the dangling pointer. Reachable from inside a default-seccomp container with **no CAP_NET_ADMIN / CAP_SYS_ADMIN** — the disclosing team reached host root in six of eight container-escape attempts. `check_type=compound` (kernel-version + read-only module gate). The socket-reachability probe is **deliberately omitted**: the engine's probe opens a live `socket(af,type,proto)`, and `socket(AF_INET, SOCK_STREAM, IPPROTO_SCTP)` would auto-load the `sctp` module — a state change that would break the read-only guarantee. Reachability is instead inferred from module state (`sctp` loaded in `/proc/modules`, or not blacklisted in `/etc/modprobe.d` and therefore auto-loadable on demand). Introduced 2.6.25 (commit 42e30bf3463c); fixed 6.6.148 / 6.12.101 / 6.18.42 / 7.1.6 (commit 9b2854f86f0b). CVSS v4.0 8.5.
- **CVE-2026-64561 (Zapscape).** Stale-root check-ordering use-after-free in KVM/x86's shadow MMU: reclaim invalidates the shadow root mid-fault, child pages inherit the invalid state, and later cleanup double-links then frees the page. An attacker with L1-guest kernel privilege (guest root) escapes to host root. `check_type=kernel_version`, **x86-arch-gated** (arm64 KVM is unaffected — see ITScape CVE-2026-46316), nested-virt only; on Intel the trigger additionally needs EPT page-walk length 4 and 5 exposed to L1. Sibling of Januscape (CVE-2026-53359) in the same shadow-MMU code family. NVD: 5.9+ affected, fixed in the same 2026-08-03 stable batch (commit 2abd5287f083). Red Hat preliminary CVSS 7.0 (CWE-825).
- **Cross-cutting.** Both fixed in the same 2026-08-03 stable release batch (6.6.148 / 6.12.101 / 6.18.42 / 7.1.6 / 7.2-rc5). SCTPhantom's NVD CPE config was not yet published at authoring, so its `upstream_ranges` encodes the introduced floor only (per-branch fixed points live in `fixed_versions`), mirroring the Januscape/ITScape treatment; re-verify against NVD when available. Both CVSS values are provisional (disclosing-team / Red Hat assessments) pending NVD publication.
...
Closes two coverage gaps found during a review against advisories published after the v4.7.2 sweep, plus one older-but-in-scope item the earlier sweeps had missed. CVE database total: **32 → 38 entries**. No new numbered check — the containerd cluster is wired into the existing check 52 runtime probe the same way the runc CVEs are, so the check count stays at 53.

- **New: 2026-06 containerd CRI-plugin cluster (AWS bulletin 2026-046).** Five CVEs in the containerd CRI plugin, affecting versions 1.7 through 2.3, all fixed in containerd 2.3.2 / 2.2.5 / 2.1.9 / 2.0.10 / 1.7.33: **CVE-2026-50195** (CVSS 8.8) — CRI checkpoint-import image-cache poisoning enabling *cross-pod code execution* on shared nodes; **CVE-2026-53488** (CVSS 8.3) — image-config LABEL propagated unsanitised to a host command context (arbitrary host command execution, needs no checkpoint/restore); **CVE-2026-53492** (CVSS 6.8) — CDI annotation smuggling from untrusted checkpoint metadata (device/host-mount injection, needs CDI enabled); **CVE-2026-53489** — checkpoint-restore symlink giving arbitrary host file read; **CVE-2026-47262** — image-triggered runtime DoS. containerd underpins EKS/ECS/Fargate/Bottlerocket/Amazon Linux, so these were squarely in scope yet absent from the database. Unlike the Leaky Vessels BuildKit siblings (CVE-2024-23651/23652/23653), which remain intentionally out of scope as image-build tooling, this cluster is CRI-runtime-level, so that exclusion does not apply.
- **Live detection via check 52, not a static advisory.** Check 52 (`check_runtime_versions`) already probed the containerd version but only printed an info line. It now caches the detected version and computes a **branch-aware** verdict (`CONTAINERD_CRI_VERDICT`): it picks the fix for the binary's own `major.minor` branch (1.7→1.7.33, 2.0→2.0.10, 2.1→2.1.9, 2.2→2.2.5, 2.3→2.3.2) and compares the patch level, so a `1.7.20` is flagged while a `2.3.2` is cleared — a single global fixed-version compare would have been wrong for a multi-branch backport. The five conf entries are `check_type=manual` with `component=containerd`, and the `manual` dispatch consults `CONTAINERD_CRI_VERDICT` exactly as it consults `RUNC_VERDICT_*` for the runc CVEs: a reachable binary yields a **definitive** CONFIRMED/not-vulnerable finding, falling back to advisory-tracking only when no containerd binary is reachable (never silently "safe"). A branch outside the 1.7–2.3 table is left UNKNOWN/verify rather than cleared. CVE-2026-46680 (containerd runAsNonRoot) deliberately keeps its own behavioural handling and is *not* routed through the new cross-reference.
- **New: CrackArmor — Qualys AppArmor LSM kernel flaws (disclosed 2026-03).** Anchor entry **CVE-2026-23268** carries the full eleven-CVE set (CVE-2026-23268/23269/23403–23411) in its `alias` field for inventory visibility. The class is a set of confused-deputy / use-after-free / double-free bugs in the kernel's AppArmor code, present since v4.11 (2017), giving local root, AppArmor-confinement removal, KASLR/kernel-memory leak and DoS — Qualys demonstrated root on Ubuntu 24.04.3 and Debian 13.1. It matters here because AppArmor is Docker's default MAC layer and **check 11 only reports AppArmor enforcement *status*, not a flaw in the AppArmor code itself**. Fixes are distro-backported (Canonical/Debian/SUSE shipped simultaneously), so the entry uses `check_type=kernel_version` with `introduced=4.11` and `vendor_defer` to the distro trackers — it resolves to *defer: consult advisory* on a tracked distro and to *unknown* (never "safe") on an unrecognised vanilla kernel, consistent with the engine's closed-world model. The confused-deputy precondition (world-writable `/sys/kernel/security/apparmor/.load|.replace|.remove`) is noted as a candidate behavioural probe for a future release.
- **VERIFY markers.** CVE-2026-53489 and CVE-2026-47262 carry `cvss=VERIFY` (the AWS bulletin did not state their scores); the exact containerd 1.7.x lower bound, CrackArmor's per-CVE NVD CVSS, and its per-distro fixed kernel package versions are likewise flagged VERIFY — confirm against the vendor advisory before relying on them for automated severity scoring, per this repo's existing convention.
...
### KVM/x86 shadow-MMU exposure (new check 60)

The x86 counterpart to check 51's arm64 ITScape probe. The two x86 KVM shadow-MMU escapes in the CVE database — Zapscape (CVE-2026-64561) and Januscape (CVE-2026-53359) — were version-only entries with no behavioural signal; check 60 adds the reachability precondition. Read-only: `uname`, `stat`/`-w` on `/dev/kvm`, `/proc/modules`, and `/sys/module/kvm_*/parameters/nested` — it never opens `/dev/kvm`, issues no KVM ioctls, and creates no VM.

| # | Check | Severity | Related CVE(s) |
|---|---|---|---|
| 60 | KVM/x86 shadow-MMU exposure — `/dev/kvm` writability / world-writability + nested-virt state (x86-arch-gated) | HIGH/MEDIUM/INFO | CVE-2026-64561 (Zapscape), CVE-2026-53359 (Januscape) |
...
| CVE-2026-50195 | containerd CRI Checkpoint Image Cache Poisoning | 8.8 | | |
| CVE-2026-53488 | containerd Image LABEL Host Command Execution | 8.3 | | |
| CVE-2026-53492 | containerd CDI Annotation Host-Mount Injection | 6.8 | | |
...
| CVE-2026-72137 | xfrm nat_keepalive double-free (LPE) | 7.8* | | |
| CVE-2026-64564 | SCTPhantom (SCTP ASCONF transport UAF; container-to-host, no CAP) | 8.5 | | |
| CVE-2026-64561 | Zapscape (KVM/x86 shadow-MMU stale-root UAF; guest→host + /dev/kvm LPE) | 7.0 | | |
| CVE-2025-31133 | runc maskedPaths /dev/null symlink container escape | 7.3 | | |
| CVE-2025-52565 | runc /dev/console bind-mount race container escape | 7.3 | | |
| CVE-2025-52881 | runc /proc write-redirect (LSM-bypass) container escape | 7.3 | | |
| CVE-2025-37947 | ksmbd streams_xattr out-of-bounds write LPE (public PoC) | 5.5 | | |
| CVE-2024-0582 | io_uring PBUF page-reuse UAF (LPE; public Exodus exploit) | 7.8 | | |
| CVE-2025-40364 | io_uring provided-buffer ring-state flaw (vendor-rated DoS) | 3.3* | | |
| CVE-2025-21863 | io_uring opcode Spectre-v1 speculation hardening gap (info-leak) | 4.7* | | |
...
> **CVE-2026-72137 (xfrm nat_keepalive double-free).** A double-free in `nat_keepalive_send()`: the IPsec NAT-keepalive skb is freed on an IPv4/IPv6 send-helper error, but once `ip_build_and_send_pkt()` / `ip6_xmit()` has taken ownership the stack may already have freed it. A controllable heap-corruption primitive → local privilege escalation; a public exploit (NebuSec) targets Ubuntu 26.04. Reaching it needs the ability to configure an xfrm/IPsec state with NAT-T keepalive and induce a send error — `CAP_NET_ADMIN`, held directly or obtained via an unprivileged user+net namespace — so it is the same container-reachability class as the net/sched act_api UAF (CVE-2026-53264), covered by the capability/namespace checks (check 44). **CVSS caveat (the `*` in the table):** the only published score is a *secondary* CVSS 3.1 of 9.8 with an `AV:N/PR:N` (remote, unauthenticated) vector — that is an automated mis-score, since the vulnerable free is on the *local transmit* path, not attacker-supplied inbound packets. The database uses a practical-impact **7.8** (local LPE, `AV:L/PR:L`) with the rationale recorded in the entry's `cvss` field, marked `VERIFY` pending an NVD primary score (NVD status "Received"). Encoded `kernel_version` with `introduced=6.10` (the nat_keepalive feature's origin) and `fixed_versions=none` + `vendor_defer` — the four stable backport commits (`226f4a490d1a` / `5b0c4c916f20` / `a8a7e6a9ff8a` / `d0a4dc7efa82`) are recorded but clean stable version strings were not published at authoring; confirm against the vendor tracker.

> **2024/2025 backfill (runc trio, ksmbd, io_uring).** The **runc trio** (CVE-2025-31133 / -52565 / -52881, Nov 2025) are `component=runc` entries — the CVE engine records the fix (runc 1.2.8 / 1.3.3 / 1.4.0-rc.3) and defers the live verdict to check 52 (runc version) and check 26 (masked-path probe), the same pattern as CVE-2019-5736 / CVE-2024-21626. **CVE-2025-37947** (ksmbd `streams_xattr` OOB write) is `compound`, module-gated on `ksmbd`; it has a public working LPE exploit (Doyensec, Ubuntu 22.04.5), vendor CVSS 5.5. **CVE-2024-0582** is the 2024 backdate — a genuine HIGH io_uring page-reuse UAF (Jann Horn / Project Zero) with a public weaponised exploit (Exodus), clean `[verified]` versions (introduced 6.4, fixed 6.6.5). The two `*`-marked io_uring rows are scored below their NVD auto-values on purpose: **CVE-2025-40364** is vendor-rated a DoS (Amazon 3.3, availability-only) and **CVE-2025-21863** is a Spectre-v1 opcode-speculation hardening gap (side-channel info-leak, Amazon 4.7), not memory-corruption LPEs — both are retained at honest LOW under a "flag it if the vulnerable code is present" posture rather than dropped, so the database stays comprehensive without letting sub-threshold bugs sit at the same severity as real escapes. **Not included:** CVE-2025-21333, which is a *Microsoft Windows Hyper-V* CVE and cannot apply to a Linux host.
...
The config-driven CVE engine additionally covers the most recent kernel privilege-escalation and container-escape issues, including ptrace credential hijack (CVE-2026-46333), nf_tables anonymous-set use-after-free (CVE-2026-23111), Fragnesia ESP (CVE-2026-46300), ITScape KVM/arm64 escape (CVE-2026-46316), Januscape KVM/x86 shadow-MMU escape (CVE-2026-53359, with its required companion fix CVE-2026-46113), an IPv6 fragmentation container escape (CVE-2026-53362), the epoll `ep_remove()` race UAF Bad Epoll (CVE-2026-46242), the futex priority-inheritance UAF GhostLock (CVE-2026-43499), the io_uring zcrx OOB write (CVE-2026-43121), the SCTP ASCONF transport UAF SCTPhantom (CVE-2026-64564, container-to-host escape with no CAP required), the KVM/x86 shadow-MMU stale-root UAF Zapscape (CVE-2026-64561, L1-guest-to-host escape; sibling of Januscape), the xfrm NAT-keepalive double-free (CVE-2026-72137, local LPE reachable via CAP_NET_ADMIN or an unprivileged user+net namespace), the two ksmbd kernel bugs (the 2022 tree-disconnect UAF CVE-2022-47939 and the 2025 `streams_xattr` out-of-bounds-write LPE CVE-2025-37947, the latter with a public Doyensec exploit), the Nov-2025 **runc trio** of container escapes (CVE-2025-31133 / -52565 / -52881, promoted from behavioural-only detection to first-class `component=runc` entries), and — as part of a **backdate of relevant container-escape and LPE CVEs to 2024** — the io_uring `IORING_REGISTER_PBUF_RING` page-reuse use-after-free (CVE-2024-0582, a genuine HIGH LPE with a public Exodus exploit) alongside two lower-severity io_uring items retained for conservative coverage at honest LOW severity (CVE-2025-40364, a vendor-rated DoS; CVE-2025-21863, a Spectre-v1 opcode-speculation hardening gap). It also tracks a set of container-runtime CVEs as advisory entries (Kata CVE-2026-41326, containerd CVE-2026-46680, Podman CVE-2026-55686, runc CVE-2026-41579 plus the historical runc escapes CVE-2019-5736 and CVE-2024-21626 "Leaky Vessels", and the classic cgroup release_agent escape CVE-2022-0492 / CISA KEV). See the [Recent CVEs](#recent-cves) detail section below.
...
Remediation: Patch the host kernel's KVM/arm64 ITS emulation. This is a hypervisor-boundary issue — relevant to nested-virt and Kata/Firecracker-style isolation on arm64, not classic namespace containers. See check 60 for the x86 equivalent.

#### Check 60 -- KVM/x86 shadow-MMU exposure (CVE-2026-64561 Zapscape / CVE-2026-53359 Januscape) (HIGH/MEDIUM/INFO)

The x86 counterpart to check 51. Both Zapscape and Januscape are use-after-frees in KVM/x86's shadow-MMU code, reached by creating a VM through `/dev/kvm`. The reach is wider than "nested-virt guest→host": on RHEL / AlmaLinux / Rocky / CloudLinux families `/dev/kvm` is world-writable (0666), so any local user — or a process in a container with `/dev/kvm` exposed — can open it, build a throwaway guest, and drive the shadow-MMU UAF against the host kernel without already being inside a nested guest. That is a direct unprivileged-to-root LPE on those distros and a container-to-host escape wherever `/dev/kvm` is reachable.

```bash
# What check 60 reads (read-only) — never opens /dev/kvm, no ioctls, no VM
uname -m
stat -c '%A %a %U:%G' /dev/kvm      # 0666 => world-writable, the RHEL-family default
test -w /dev/kvm && echo "writable from this context"
cat /sys/module/kvm_intel/parameters/nested 2>/dev/null   # or kvm_amd
```

Remediation: Do not expose `/dev/kvm` to application containers — remove the device mount. On RHEL-family hosts, restrict `/dev/kvm` to a trusted `kvm` group (udev rule, mode 0660) rather than the default 0666. Patch the host kernel for both CVEs: Zapscape commit `2abd5287f083`, Januscape commit `81ccda30b4e8` plus its companion CVE-2026-46113 (`0cb2af2ea66a`). Where nested virtualization isn't required, disable it (`kvm_intel`/`kvm_amd` `nested=0`).
